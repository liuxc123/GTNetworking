...
FOUNDATION_EXPORT NSString *const GTRequestValidationErrorDomain;

NS_ENUM(NSInteger) {
    GTRequestValidationErrorInvalidStatusCode = -8,
    GTRequestValidationErrorInvalidJSONFormat = -9,
};

///  HTTP Request method.
typedef NS_ENUM(NSInteger, GTRequestMethod) {
    GTRequestMethodGET = 0,
    GTRequestMethodPOST,
    GTRequestMethodHEAD,
    GTRequestMethodPUT,
    GTRequestMethodDELETE,
    GTRequestMethodPATCH,
};

///  Request serializer type.
typedef NS_ENUM(NSInteger, GTRequestSerializerType) {
    GTRequestSerializerTypeHTTP = 0,
    GTRequestSerializerTypeJSON,
};

///  Response serializer type, which determines response serialization process and
///  the type of `responseObject`.
typedef NS_ENUM(NSInteger, GTResponseSerializerType) {
    /// NSData type
    GTResponseSerializerTypeHTTP,
    /// JSON object type
    GTResponseSerializerTypeJSON,
    /// NSXMLParser type
    GTResponseSerializerTypeXMLParser,
};

///  Request priority
typedef NS_ENUM(NSInteger, GTRequestPriority) {
    GTRequestPriorityLow = -4L,
    GTRequestPriorityDefault = 0,
    GTRequestPriorityHigh = 4,
};
...
typedef void(^GTRequestCompletionBlock)(__kindof GTBaseRequest *request);

///  The GTRequestDelegate protocol defines several optional methods you can use
...
- (void)requestFinished:(__kindof GTBaseRequest *)request;

///  Tell the delegate that the request has failed.
///
///  @param request The corresponding request.
- (void)requestFailed:(__kindof GTBaseRequest *)request;

@end

///  The GTRequestAccessory protocol defines several optional methods that can be
...
@protocol GTRequestAccessory <NSObject>
...
///  GTBaseRequest is the abstract class of network request. It provides many options
///  for constructing request. It's the base class of `GTRequest`.
@interface GTBaseRequest : NSObject
...
///  `GTResponseSerializerType`. Note this value can be nil if request failed.
///
...
///  If you use `GTResponseSerializerTypeJSON`, this is a convenience (and sematic) getter
...
///  Default is nil.
@property (nonatomic, weak, nullable) id<GTRequestDelegate> delegate;
...
@property (nonatomic, copy, nullable) GTRequestCompletionBlock successCompletionBlock;

///  The failure callback. Note if this value is not nil and `requestFailed` delegate method is
///  also implemented, both will be executed but delegate method is first called. This block
///  will be called on the main queue.
@property (nonatomic, copy, nullable) GTRequestCompletionBlock failureCompletionBlock;
...
///  The priority of the request. Effective only on iOS 8+. Default is `GTRequestPriorityDefault`.
@property (nonatomic) GTRequestPriority requestPriority;

///  Set completion callbacks
- (void)setCompletionBlockWithSuccess:(nullable GTRequestCompletionBlock)success
                              failure:(nullable GTRequestCompletionBlock)failure;
...
- (void)startWithCompletionBlockWithSuccess:(nullable GTRequestCompletionBlock)success
                                    failure:(nullable GTRequestCompletionBlock)failure;

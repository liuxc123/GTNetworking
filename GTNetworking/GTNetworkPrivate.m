...
void GTLog(NSString *format, ...) {
#ifdef DEBUG
    if (![GTNetworkConfig sharedConfig].debugLogEnabled) {
        return;
    }
    va_list argptr;
...
+ (void)addDoNotBackupAttribute:(NSString *)path {
    ...
    if (error) {
        GTLog(@"error to set do not backup attribute, error = %@", error);
    }
}
...
@implementation GTBaseRequest (RequestAccessory)

- (void)toggleAccessoriesWillStartCallBack {
    for (id<GTRequestAccessory> accessory in self.requestAccessories) {
...
@implementation GTBatchRequest (RequestAccessory)
...
@implementation GTChainRequest (RequestAccessory)
